10 ' === LED MATH LAB — 10 Patterns + Cycle Mode (MBASIC-80 Rev 5.21) ===
20 ' SC7xx LEDs on I/O port (edit LEDPORT if needed)
30 LEDPORT = &H00
40 RANDOMIZE
50 DIM P(7) : P(0)=1: FOR I=1 TO 7: P(I)=P(I-1)*2: NEXT I
60 PRINT "LED MATH LAB  —  SC7xx LEDs on port &H00 (edit line 30 for yours)"
...
990 NEXT LOOP
995 RETURN
' sub to prime step counters in each mode for cycle
9500 CSTEP=0: RETURN

' ===== 1) LFSR-8 (taps 0,2,3,4) =====
1000 PRINT : PRINT "1) LFSR-8  — Q to quit"
...
' ===== 2) Rule-30 CA (8-bit ring) =====
2000 PRINT : PRINT "2) Rule-30 CA  — Q to quit"
...
' ===== 3) Logistic map fixed-point =====
3000 PRINT : PRINT "3) Logistic map  — Q to quit"
...
' ===== 4) Binary counter =====
4000 PRINT : PRINT "4) Counter  — Q to quit"
4010 V=0
4020 OUT LEDPORT, V
...
' ===== 5) Rotating one-hot ring =====
5000 PRINT : PRINT "5) RotRing  — Q to quit"
5010 V=1
5020 OUT LEDPORT, V
...
' ===== 6) KITT scanner (bouncing one-hot) =====
...
6050 POS=POS+DIR
6060 IF POS>=7 THEN POS=7: DIR=-1
6070 IF POS<=0 THEN POS=0: DIR=1
6080 IF M=0 THEN CSTEP=CSTEP+1: IF CSTEP>=STEPS THEN RETURN
6090 GOTO 6020

' ===== 7) Sparkle (decay + random sparks) =====
7000 PRINT : PRINT "7) Sparkle  — Q to quit"
7010 W=0
7020 W=INT(W/2) ' decay (shift right)
7030 IF RND>.66 THEN K=INT(RND*8): T=INT(W/P(K)): B=T-2*INT(T/2): IF B=0 THEN W=W+P(K)
...
' ===== 8) Rule-110 CA (8-bit ring) =====
8000 PRINT : PRINT "8) Rule-110 CA  — Q to quit"
8010 W=85 ' 01010101 seed (lively)
8020 NEXTW=0
8030 FOR I=0 TO 7
8040 T=INT(W/P(I)): SBIT=T-2*INT(T/2)
8050 LI=I+1: IF LI=8 THEN LI=0
8060 T=INT(W/P(LI)): LBIT=T-2*INT(T/2)
8070 RI=I-1: IF RI<0 THEN RI=7
8080 T=INT(W/P(RI)): RBIT=T-2*INT(T/2)
8090 CODE=LBIT*4 + SBIT*2 + RBIT
8100 NB=1
8110 IF CODE=0 THEN NB=0 ELSE IF CODE=4 THEN NB=0 ELSE IF CODE=7 THEN NB=0
8120 NEXTW=NEXTW + NB*P(I)
8130 NEXT I
8140 W=NEXTW
8150 OUT LEDPORT, W
...
' ===== 9) Bit-reversed counter =====
9000 PRINT : PRINT "9) BitReverse  — Q to quit"
9010 T=0
...
' ===== 10) DualRing (two one-hot, opposite directions) =====
10000 PRINT : PRINT "10) DualRing  — Q to quit"
10010 POS1=0: POS2=7
