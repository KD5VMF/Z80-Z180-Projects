...
public sealed class MainForm : Form
{
    ...
    private readonly RichTextBox _hexBox = new();
    private readonly RichTextBox _logBox = new();
    private readonly TabControl _sourceTabs = new();
    private readonly TabPage _asmTab = new("ASM Source");
    private readonly TabPage _hexTab = new("HEX Paste");
    ...
    public MainForm()
    {
        ...
        Text = "TMS9918A ASM Builder IDE for SC720 / RomWBW CP/M - REV8";
        Width = 1280;
        Height = 860;
        ...
        var btnTemplate = Btn("Load Balls Demo", (_, _) => LoadBallsDemo());
        var btnBuild = Btn("Build .COM + .HEX", async (_, _) => await BuildAsync());
        var btnCopyHex = Btn("Copy HEX", (_, _) => CopyHexToClipboard());
        ...
        {
            btnNew, btnOpen, btnSave, btnTemplate, btnBuild, btnCopyHex, btnLibs, btnImportLibs,
        ...
        _hexBox.Multiline = true;
        _hexBox.ScrollBars = RichTextBoxScrollBars.Both;
        _hexBox.WordWrap = false;
        _hexBox.ReadOnly = true;
        _hexBox.DetectUrls = false;
        _hexBox.BorderStyle = BorderStyle.None;
        _hexBox.Font = new Font("Consolas", 10);
        _hexBox.BackColor = Color.FromArgb(12, 12, 12);
        _hexBox.ForeColor = Color.LightGreen;
        _hexBox.Dock = DockStyle.Fill;
        _hexBox.Text = string.Empty;

        _sourceTabs.Dock = DockStyle.Fill;
        _asmTab.Controls.Add(_asmBox);
        _hexTab.Controls.Add(_hexBox);
        _sourceTabs.TabPages.Add(_asmTab);
        _sourceTabs.TabPages.Add(_hexTab);
        ...
        split.Panel1.Controls.Add(_sourceTabs);
        split.Panel2.Controls.Add(_logBox);

        _status.Dock = DockStyle.Bottom;
        _status.Height = 24;
        _status.Text = "Ready. No ASM loaded. Click New ASM, Open ASM, or Load Balls Demo.";
        _status.BorderStyle = BorderStyle.Fixed3D;

        Controls.Add(split);
        Controls.Add(_status);
        Controls.Add(top);
        Log("TMSAsmBuilder REV8 ready.", LogKind.Header);
        Log("This builds J.B. Langston-style Z80 source on Windows, then makes a CP/M .COM and Intel HEX for your SC720.");
        Log("No program is loaded on startup. Click New ASM, Open ASM, or Load Balls Demo.");
    ...
    private void ColorizeAsm()
    {
        ...
        var scrollPos = new NativePoint();

        // RichTextBox syntax coloring changes Selection many times. Without WM_SETREDRAW,
        // every edit can visibly repaint the whole editor/terminal area. Keep the window
        // frozen, restore caret/scroll, then invalidate once at the end.
        if (_asmBox.IsHandleCreated)
        {
            SendMessage(_asmBox.Handle, EM_GETSCROLLPOS, IntPtr.Zero, ref scrollPos);
            SendMessage(_asmBox.Handle, WM_SETREDRAW, IntPtr.Zero, IntPtr.Zero);
        }
        ...
        finally
        {
            ...
            if (_asmBox.IsHandleCreated)
            {
                SendMessage(_asmBox.Handle, EM_SETSCROLLPOS, IntPtr.Zero, ref scrollPos);
                SendMessage(_asmBox.Handle, WM_SETREDRAW, new IntPtr(1), IntPtr.Zero);
                _asmBox.Invalidate();
            }
    ...
    private void NewAsm()
    {
        ...
        _logBox.Clear();
        _hexBox.Clear();
        _sourceTabs.SelectedTab = _asmTab;
    ...
    private void LoadBallsDemo()
    {
        var templatePath = Path.Combine(_root, "Templates", "BALLS.ASM");
        if (File.Exists(templatePath))
            _asmBox.Text = ToWinLines(File.ReadAllText(templatePath));
        else
            _asmBox.Text = ToWinLines(BallsTemplate());

        _fileBox.Text = Path.Combine(_work, "BALLS.ASM");
        _outBox.Text = "BALLS.COM";
        _hexBox.Clear();
        _sourceTabs.SelectedTab = _asmTab;
        Log("Loaded Balls Demo: four colored 16x16 sprites bouncing on a black TMS9918A/TMSEMU3 screen.");
        _status.Text = "Balls Demo loaded. Output is BALLS.COM. Click Build .COM + .HEX.";
    }
    ...
    private async Task DownloadLibsAsync()
    {
        ...
        {
            Log("DOWNLOAD FAILED: " + ex.Message, LogKind.Error);
    ...
    private async Task BuildAsync()
    {
        ...
        try
        {
            ...
            {
                MessageBox.Show(this,
                    "No ASM program is loaded. Click New ASM and type/paste code, Open ASM, or Load Balls Demo first.",
            ...
            var args = _argsBox.Text.Replace("{src}", AssemblerArgPath(tempAsm)).Replace("{out}", AssemblerArgPath(tempCom));
            Log("--- BUILD START ---", LogKind.Header);
            ...
            {
                Log("Assembler output:", LogKind.Header);
                LogBuildOutput(stdout.TrimEnd());
            }
            if (!string.IsNullOrWhiteSpace(stderr))
            {
                Log("Assembler warnings/errors:", LogKind.Warning);
                LogBuildOutput(stderr.TrimEnd());
            }
            Log("Exit code: " + p.ExitCode, p.ExitCode == 0 ? LogKind.Success : LogKind.Error);
            if (p.ExitCode != 0)
                Log("FAILED COMPILE: assembler returned a non-zero exit code. Read the red/yellow lines above first.", LogKind.Error);
            ...
            if (File.Exists(tempCom))
            {
                ...
                Log("SUCCESS: temp .COM built (" + len + " bytes) and converted to Intel HEX.", LogKind.Success);
                ...
                Log("HEX tab is now loaded with the full Intel HEX text, ready to copy/paste into Tera Term.", LogKind.Success);
                _status.Text = "Built clean .ASM + .HEX folder " + Path.GetFileName(buildDir) + "; HEX tab is ready; .COM is in Out.";
                MessageBox.Show(this,
                    "Built clean .ASM + .HEX output.\n\nBuild folder only contains the new ASM and HEX files:\n" + buildDir + "\n\nLatest .COM copy is in Out for XM if needed.\n\nThe HEX tab now contains the full paste-ready Intel HEX text.\n\nPIP/LOAD commands:\nPIP " + hexName + "=CON:\nLOAD " + Path.GetFileNameWithoutExtension(outName).ToUpperInvariant(),
                    "Build complete", MessageBoxButtons.OK, MessageBoxIcon.Information);
                OpenFolder(buildDir);
            }
            else
            {
                Log("FAILED: no .COM output was created in the internal assembler work folder. Check the log and assembler argument template.", LogKind.Error);
                _status.Text = "Build failed. See log.";
            }
            Log("--- BUILD END ---", LogKind.Header);
        }
        catch (OperationCanceledException)
        {
            Log("Build cancelled.", LogKind.Warning);
        }
        catch (Exception ex)
        {
            Log("BUILD ERROR: " + ex, LogKind.Error);
        ...
        finally
        {
            if (!string.IsNullOrWhiteSpace(tempDir))
            {
                ...
                {
                    Log("Temp cleanup warning: " + cleanupEx.Message, LogKind.Warning);
                }
            }
        }
    }

    private static string AssemblerArgPath(string path) => Path.GetFullPath(path).Replace('\\', '/');

    private void SetHexDump(string hexPath)
    {
        try
        {
            _hexBox.Text = ToWinLines(File.ReadAllText(hexPath, Encoding.ASCII));
            _sourceTabs.SelectedTab = _hexTab;
        }
        catch (Exception ex)
        {
            Log("HEX TAB WARNING: built HEX file could not be loaded into the HEX tab: " + ex.Message, LogKind.Warning);
        }
    }

    private void ClearHexDump()
    {
        if (_isColorizing || _hexBox.IsDisposed || _hexBox.TextLength == 0) return;
        _hexBox.Clear();
    }
    ...
    private void CopyHexToClipboard()
    {
        try
        {
            ...
            {
                MessageBox.Show(this,
                    "No HEX dump is loaded yet. Build a program first, then the HEX tab will fill with paste-ready Intel HEX.",
                    "No HEX to copy", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            Clipboard.SetText(ToWinLines(_hexBox.Text));
            Log("Copied HEX dump to clipboard. In CP/M use: PIP NAME.HEX=CON: then paste in Tera Term, finish with Ctrl+Z, then LOAD NAME.", LogKind.Success);
            _status.Text = "HEX dump copied to clipboard for Tera Term paste.";
        }
        catch (Exception ex)
        {
            Log("COPY HEX FAILED: " + ex.Message, LogKind.Error);
            MessageBox.Show(this, ex.Message, "Copy HEX failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
    ...
    private void LogBuildOutput(string text)
    {
        foreach (var rawLine in text.Replace("\r\n", "\n").Replace('\r', '\n').Split('\n'))
        {
            var line = rawLine.TrimEnd();
            Log(line, ClassifyBuildLine(line));
        }
    }

    private static LogKind ClassifyBuildLine(string line)
    {
        if (string.IsNullOrWhiteSpace(line)) return LogKind.ToolOutput;
        if (Regex.IsMatch(line, @"\b(error|failed|fatal|undefined|not found|cannot open|invalid)\b", RegexOptions.IgnoreCase))
            return LogKind.Error;
        if (Regex.IsMatch(line, @"\b(warn|warning|caution|deprecated)\b", RegexOptions.IgnoreCase))
            return LogKind.Warning;
        if (Regex.IsMatch(line, @"\b(pass|success|succeeded|built|created)\b", RegexOptions.IgnoreCase))
            return LogKind.Success;
        return LogKind.ToolOutput;
    }

    private void Log(string msg, LogKind kind = LogKind.Info)
    {
        if (_logBox.IsDisposed) return;
        if (_logBox.InvokeRequired)
        {
            _logBox.BeginInvoke(new Action(() => Log(msg, kind)));
            return;
        }

        msg = msg.Replace("\r\n", "\n").Replace('\r', '\n');
        foreach (var rawLine in msg.Split('\n'))
            AppendLogLine(rawLine, kind == LogKind.Info ? ClassifyBuildLine(rawLine) : kind);
    }

    private void AppendLogLine(string line, LogKind kind)
    {
        var oldStart = _logBox.SelectionStart;
        var oldLen = _logBox.SelectionLength;
        var shouldFollow = _logBox.SelectionStart >= _logBox.TextLength - 2;

        _logBox.SelectionStart = _logBox.TextLength;
        _logBox.SelectionLength = 0;
        _logBox.SelectionColor = LogColor(kind);
        _logBox.SelectionFont = LogFont(kind);
        ...
        else if (oldStart <= _logBox.TextLength)
        {
            _logBox.Select(oldStart, Math.Min(oldLen, _logBox.TextLength - oldStart));
        }
    }

    private Font LogFont(LogKind kind)
    {
        if (kind is LogKind.Error or LogKind.Warning or LogKind.Header or LogKind.Success)
        {
            _logBoldFont ??= new Font(_logBox.Font, FontStyle.Bold);
            return _logBoldFont;
        }
    ...
    private static Color LogColor(LogKind kind) => kind switch
    {
        LogKind.Header => Color.DeepSkyBlue,
        LogKind.Success => Color.LightGreen,
        LogKind.Warning => Color.Gold,
        LogKind.Error => Color.LightCoral,
        LogKind.ToolOutput => Color.Silver,
        _ => Color.Gainsboro
    };

    private static string NewTemplate() => string.Empty;

    private static string BallsTemplate() => @"; BALLS.ASM
