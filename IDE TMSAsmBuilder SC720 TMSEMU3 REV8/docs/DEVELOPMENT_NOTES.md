...
The editor intentionally starts blank. This prevents a user from accidentally thinking the Balls Demo is their own active project. The user chooses the source:
...
`Load Balls Demo` reads `TMSAsmBuilder/Templates/BALLS.ASM`. If that file is missing from a custom build, the app has an embedded fallback copy of the demo.
